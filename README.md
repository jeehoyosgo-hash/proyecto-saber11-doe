# Proyecto Investigativo — Saber 11: Muestreo & DOE

**Universidad Santo Tomás · Estadística V Semestre · 2026-I**  
Julián Toloza · Yeison Hoyos

## Descripción
Análisis integrado de muestreo estadístico y diseño de experimentos 
sobre resultados de la prueba Saber 11 (ICFES).

## Archivos
- `Proyecto_Saber11_DOE.Rmd` — Notebook reproducible (entregable principal)
- `dashboard_saber11.html` — Dashboard de presentación
- `simulador_estadistico.html` — Simulador estadístico interactivo

## Dashboard en vivo
[Ver dashboard](https://jeehoyosgo-hash.github.io/proyecto-saber11-doe/dashboard_saber11.html)
[Ver simulador](https://jeehoyosgo-hash.github.io/proyecto-saber11-doe/simulador_estadistico.html)


## Datos
Fuente: [ICFES - Datos Abiertos Colombia](https://www.datos.gov.co/resource/kgxf-xxbe)# proyecto-saber11-doe
